...
int main()
{
	...
	cout<<"Enter the number of segments u wnat to split"<<endl;		//segments should be atlest 1 more then processes to work properply
	cin>>s;
	...
	for(int t=0;t<n;t++)
	{
		...
		{
			sf[new_min].flag=0;
			cout<<"new min in else is "<<new_min<<endl;
			cout<<"fragmentation of process "<<t<<" is "<<sf[new_min].size-arr[t]<<endl;
					cout<<"mem of process "<<t<<" is "<<sf[new_min].size<<endl;
